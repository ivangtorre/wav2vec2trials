...
def main():
    ...
    assert isinstance(training_args.num_train_epochs, object)
    training_args.output_dir = training_args.output_dir + "/" + "EPOCH-" + str(training_args.num_train_epochs) + "_LR-" + str(training_args.learning_rate) +"__date__"+ str(time.time().__round__())
...
#######################################################################################
    train_dataset = Dataset.load_from_disk("tmp_train")
    eval_dataset = Dataset.load_from_disk("tmp_test")
    #################################################################################################################
    ...
    if training_args.local_rank == 0:
        wandb.init(project="wav2vec2aphasia", config=parser.parse_args())
    ## Speed Augment
    ...
    if training_args.do_eval:
        ...
        metrics = {"cer": metrics["eval_cer"], "wer": metrics["eval_wer"]}
        if training_args.local_rank == 0:
        ...
        if training_args.local_rank == 0:
            artifact = wandb.Artifact(name=f"model-{wandb.run.id}", type="model", metadata={"wer": metrics["wer"]})
